        ...
        for file in files:
            if file.lower().endswith(image_extensions):
                ...
                if len(result) > 0 and len(result[0]) > 1:
                    # 获取第一个文本块的识别文字
                    first_text_block_text = result[0][0][1][0]
                    # 转换为简体字
                    simplified_text = cc.convert(first_text_block_text)
                    ...
                    for t, r in zip(target_text, simplified_text):
                        if t != r:
                            diff += f"目标：{t}，识别：{r}；"
                    line = f"{relative_path}：{first_text_block_text}\n"
                    f.write(line)
                    html_table += f"<tr><td>{relative_path}</td><td>{target_text}</td><td>{simplified_text}</td><td>{diff}</td></tr>"
                    print(f"已处理 {processed_images + 1}/{total_images}：{relative_path}，识别文字：{first_text_block_text}")
                else:
                    line = f"{relative_path}：未检测到文本块\n"
                    f.write(line)
                    html_table += f"<tr><td>{relative_path}</td><td>{target_text}</td><td>未检测到文本块</td><td>未检测到文本块</td></tr>"
                    print(f"已处理 {processed_images + 1}/{total_images}：{relative_path}，未检测到文本块")
