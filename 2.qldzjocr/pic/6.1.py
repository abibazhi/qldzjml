        ...
        for file in files:
            if file.lower().endswith(image_extensions):
                ...
                if found_text_block is not None:
                    line = f"{relative_path}：{found_text_block}\n"
                    f.write(line)
                    print(f"已处理 {processed_images + 1}/{total_images}：{relative_path}，识别文字：{found_text_block}")
                else:
                    line = f"{relative_path}：未检测到符合条件的文本块\n"
                    f.write(line)
                    print(f"已处理 {processed_images + 1}/{total_images}：{relative_path}，未检测到符合条件的文本块")
