def compare_files_with_sliding_window_levels(sutra_list_file, extracted_info_file, output_file):
    ...
    def find_matches(text_block, sutra_name):
        """查找指定文本块与经文名称之间的匹配项"""
        matches = []
    ...
    for sutra_name, sutra_line_number in sutra_lines:
        for file_path, text_blocks in extracted_info.items():
            for text_block in text_blocks:
                matches = find_matches(text_block, sutra_name)
                if matches:
                    for match_partial, match_length in matches:
                        comparison_results[file_path].append((sutra_name, sutra_line_number, match_partial, match_length))
    ...
    with open(output_file, 'w', encoding='utf-8') as outfile:
        for file_path, matches in comparison_results.items():
            if matches:
                ...
                for sutra_name, sutra_line_number, match_partial, match_length in matches:
                    outfile.write(f"  匹配到的标准文本: \"{sutra_name}\" (行号: {sutra_line_number}), 匹配部分: \"{match_partial}\", 匹配长度: {match_length}\n")
            else:
