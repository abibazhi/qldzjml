def convert_result_to_html(result_file, html_output_file):
    ...
    html_content += "<tr style='background-color:#f2f2f2;'><th>图片路径</th><th>匹配到的标准文本</th><th>匹配到的行号</th><th>匹配部分</th><th>匹配长度</th><th>文本</th></tr>"
    ...
    for idx, line in enumerate(lines):
        ...
        if "(匹配到的标准文本:" in line:
            ...
            # 提取匹配部分和匹配长度
            if "匹配部分:" in match_info:
                match_part = match_info.split("匹配部分: ")[1].split(", 匹配长度: ")[0].replace("\"", "")
                match_length = match_info.split("匹配长度: ")[1].split(")")[0]
            else:
                match_part = "-"
                match_length = "-"
            ...
            html_content += f"<tr style='{row_style}'><td>{img_path}</td><td>{standard_text}</td><td>{line_number}</td><td>{match_part}</td><td>{match_length}</td><td>{text_block}</td></tr>"
        else:
            # 如果没有匹配信息，则只显示图片路径、文本块，并在其他列中放置破折号（-）
            row_style = f"background-color: {'#e6f7ff' if idx % 2 == 0 else '#ffffff'};"
            html_content += f"<tr style='{row_style}'><td>{img_path}</td><td>-</td><td>-</td><td>-</td><td>-</td><td>{text_block}</td></tr>"
