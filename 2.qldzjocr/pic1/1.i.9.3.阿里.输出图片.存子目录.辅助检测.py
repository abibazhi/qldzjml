...
def write_to_file(file_path, img_path, text_block_count, detected_texts, secondary_text_block_count=None, secondary_detected_texts=None):
    """将信息写入文件"""
    with open(file_path, 'a') as file:  # 使用'a'模式以追加方式打开文件
        primary_info = f"图片路径: {img_path}, 主区域检测到的文本块总数: {text_block_count}, 文本内容: {', '.join(detected_texts)}"
