...
def detect_text_in_image_part1(image_path, progress_file, output_file, temp_dir):
    try:
        with Image.open(image_path) as img:
            width, height = img.size
...
def main1(root_dir, progress_file='progress.txt', output_file='output.txt', temp_dir='temp_images'):
    if not os.path.exists(temp_dir):
        os.makedirs(temp_dir)
