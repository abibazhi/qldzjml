...
- 功能2：如果发现某个图片中，左侧有文字块识别出来，而且文字长度大于1，而且右侧的亮度==255的，记录其路径和文件名，并保存到输出文件中(output.txt)。
    ...
    try:
        with Image.open(image_path) as img:
            ...
            if part == 'left':
                ...
                detected_texts = [line[1][0] for line in result if isinstance(line, list) and len(line) > 1 and isinstance(line[1], list) and len(line[1]) > 0 and isinstance(line[1][0], str)]
                if detected_texts and max(len(text) for text in detected_texts) > 1:
                    print(f"Detected texts on left part of {image_path}: {detected_texts}")
                    has_text = True
                else:
                    has_text = False
            else:  # right part
                has_text = brightness == 255

            if part == 'right' and has_text:
                with open(output_file, 'a') as out_f:
                    out_f.write(f"Right part is very bright (brightness=255) and left part has text longer than 1 char in file: {image_path}\n")
