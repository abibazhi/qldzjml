...
def compare_files_with_sliding_window(ocr_file, html_file, output_file):
    ...
    for ocr_idx, ocr_line in enumerate(ocr_lines):
        ...
        if ocr_idx in full_matches:
            ...
            result_lines.append(f"./selected_images/{filename}, {ocr_text} (匹配到的标准文本: \"{html_line}\" (行号: {html_idx}))")
        else:
            result_lines.append(f"./selected_images/{filename}, {ocr_text}")
    ...
    for window_size in range(10, 4, -1):  # 滑动窗口大小从10递减到5
        ...
        for ocr_idx, (html_idx, html_line, partial_text, size) in partial_matches.items():
            original_line = result_lines[ocr_idx]
            result_lines[ocr_idx] += f" (匹配到的标准文本: \"{html_line}\" (行号: {html_idx}), 匹配部分: \"{partial_text}\", 匹配长度: {size})"

    # 处理未匹配的行
    for idx, line in enumerate(result_lines):
        if "匹配到的标准文本" not in line:
            result_lines[idx] += " | 没有匹配"
