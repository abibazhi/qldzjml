...
def filter_non_ascii_and_short_lines(input_path, output_path, dest_img_dir):
        ...
        for line in infile:
            ...
            if len(parts) == 2:
                ...
                if not is_ascii(text) and len(text.strip()) > 2:
                    ...
                    # 构造新的图片名称并复制图片
                    relative_path_parts = img_path.split(os.sep)
                    new_img_name = "_".join(relative_path_parts) + os.path.splitext(img_path)[1]
                    source_img_path = os.path.join(os.getcwd(), img_path)
                    dest_img_path = os.path.join(dest_img_dir, new_img_name)
                    
                    print(f"尝试复制图片: {source_img_path} 到 {dest_img_path}")
                    
                    if os.path.exists(source_img_path):
                        shutil.copy(source_img_path, dest_img_path)
                    else:
                        print(f"警告: 图片路径 {source_img_path} 不存在.")
...
filter_non_ascii_and_short_lines(input_path, output_path, dest_img_dir)
