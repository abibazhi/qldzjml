...
def filter_non_ascii_lines(input_path, output_path):
    """过滤掉文本为纯ASCII码的行，并去除空行"""
    with open(input_path, 'r', encoding='utf-8') as infile, \
        ...
        for line in infile:
            ...
            if len(parts) == 2:
                img_path, text = parts
                if not is_ascii(text):  # 仅保留非纯ASCII文本的行
...
filter_non_ascii_lines(input_path, output_path)
