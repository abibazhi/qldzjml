...
for result_line in recognition_results:
    ...
    # 检查是否为特殊处理的情况
    if relative_path in special_cases.keys():
        simplified_recognized_text = cc.convert(recognized_text)

        if relative_path.startswith('020'):  # 处理020/329.png的情况
            html_table += (f"<tr>"
                           ...
                           f"<td>无需比较</td>"
                           f"</tr>")
        elif relative_path.startswith(('038', '039')):  # 处理038和039目录下的特殊情况
            num_entries = special_cases[relative_path]
