...
source_dir = 'qldzjpng'
# 目标目录
destination_dir = 'qldzjpng_filtered'

# 定义文件大小过滤阈值（50KB）
file_size_threshold = 50 * 1024
...
for relative_dir, files_info in all_info.items():
    ...
    for filename in files_info.keys():
        ...
        try:
            ...
            print(f"文件 {source_file_path} 的大小为: {file_size} 字节")
            if file_size > file_size_threshold:
                ...
                print(f"文件 {source_file_path} 大小小于 50KB，跳过拷贝。")
        except Exception as e:
            print(f"处理文件 {source_file_path} 时出错: {str(e)}")
