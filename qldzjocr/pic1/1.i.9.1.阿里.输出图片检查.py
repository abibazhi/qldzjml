...
def copy_image_to_temp(img_path, temp_dir):
    """将图片复制到临时目录"""
    os.makedirs(temp_dir, exist_ok=True)  # 确保临时目录存在
    filename = os.path.basename(img_path)
    dest_path = os.path.join(temp_dir, filename)
    shutil.copy(img_path, dest_path)
    print(f"已将图片 {img_path} 复制到 {dest_path}")

def process_image(img_path, output_file, temp_dir):
    try:
        ...
        with Image.open(img_path) as image:
            ...
            if text_block_count > 0:
                ...
                # 如果有文本被检测到，则将图片复制到临时目录
                copy_image_to_temp(img_path, temp_dir)
...
def main(root_dir, output_file, temp_dir):
    for subdir, _, files in os.walk(root_dir):
        for file in files:
            if file.lower().endswith(('png', 'jpg', 'jpeg')):
                img_path = os.path.join(subdir, file)
                process_image(img_path, output_file, temp_dir)

if __name__ == "__main__":
    root_directory = "selected_images"  # 修改为你需要遍历的目录
    output_file = "detected_texts_output.txt"  # 输出文件名
    temp_directory = "detected_images_temp"  # 临时目录名
    main(root_directory, output_file, temp_directory)
