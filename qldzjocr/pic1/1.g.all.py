...
- 功能2：如果发现某个图片中，左侧是黑底白字的，记录其路径和文件名，并保存到输出文件中(output.txt)。
- 功能3：为了防止程序意外中止后可以继续，请记录识别的进度(progress.txt)，以便从中断的地方继续执行。
- 需求1：截取的图片不需要保存。
- 需求2：在控制台上输出处理进度，但不要输出调试信息。
...
def detect_text_in_image_part(image_path, part, progress_file, output_file):
    try:
        with Image.open(image_path) as img:
            ...
            box = (width*0//15, height*2//10, width*2//15, height*5//10) if part == 'left' else \
                  (width - width*2//15, height*2//10, width - width*0//15, height*5//10)
            
            cropped_img = img.crop(box)
            brightness = get_image_brightness(cropped_img)

            if part == 'left' and brightness < 128:
                with open(output_file, 'a') as out_f:
                    out_f.write(f"Dark background with light text in file: {image_path}\n")
