    ...
    try:
        ...
        with Image.open(image_path) as img:
            ...
            if part == 'left':
                ...
                box = (width - width*2//15, height*2//10, width - width//15, height*5//10)
            else:
            ...
            if result is None or len(result) < 1 or result[0] is None:
                ...
                has_text = True
                detected_texts = [line[1][0] for line in result if isinstance(line[1], list) and len(line[1]) > 0]
            
            if has_text:
                print(f"{part.capitalize()} part has text: {' '.join(str(text) for text in detected_texts)}")
            else:
...
if __name__ == "__main__":
    img_path = "qldzjpng/001/166.png"  # 确保此路径指向正确的图片
