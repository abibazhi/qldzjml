...
def process_image(img_path, output_file, detected_dir):
    try:
        ...
        with Image.open(img_path) as image:
            ...
            # 计算主区域检测到的文本总长度
            total_length_primary = sum(len(text.strip()) for text in detected_texts_primary)

            if text_block_count_primary > 0 and total_length_primary == 0:
                # 如果主区域检测到文本块但总长度为0，则检查辅助区域
                box_secondary = (850, 1450, 1650, 1580)
                ...
                if text_block_count_secondary > 0:
            ...
            elif text_block_count_primary > 0 and total_length_primary > 0:
                # 如果主区域检测到有效文本
