...
def process_image(img_path, temp_dir_base):
    try:
        ...
        if result is None or len(result) < 1 or result[0] is None:
            print(f"警告: 图片 {img_path} 未检测到任何文本块")
            return
        
        total_text_blocks = len(result[0])
        dark_background_count = 0
        ...
        # 只有当存在暗背景文本块时，才记录总数并保存图片
        if dark_background_count > 0:
            with open(os.path.join(temp_dir_base, 'dark_background_files.txt'), 'a') as file:
                file.write(f"图片路径: {img_path}, 总文本块数: {total_text_blocks}, 暗背景文本块数: {dark_background_count}\n")
            ...
            output_image_name = f"{original_filename.split('.')[0]}_with_boxes.jpg"
            output_image_path = os.path.join(temp_dir, output_image_name)
            cropped_image.save(output_image_path)
            print(f"已保存带有标注框的图片: {output_image_path}")
