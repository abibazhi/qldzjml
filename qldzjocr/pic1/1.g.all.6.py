...
def detect_text_in_image_part(image_path, progress_file, output_file, temp_dir):
    try:
        with Image.open(image_path) as img:
            ...
            detected_texts = []
            for item in result:
                if isinstance(item, list) and len(item) > 0 and isinstance(item[0], list) and len(item[0]) == 4 and isinstance(item[1], tuple) and len(item[1]) == 2:
                    bbox = [point for point_list in item[0] for point in point_list]  # 文字块的边界框
                    text = item[1][0]
                    confidence = item[1][1]
                    ...
                    print(f"Detected text block in file: {image_path}, Text: {text}, Confidence: {confidence}, Brightness: {brightness}")
