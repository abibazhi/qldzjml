...
def process_images_in_directory(sub_dir):
    """
    ...
    for filename in os.listdir(sub_dir):
        if filename.endswith(('.png', '.jpg', '.jpeg')):
            ...
            if (left_has_text and not right_has_text) or (not left_has_text and not right_has_text and middle_has_text):
                cover_files.append(filepath)
            elif not left_has_text and not middle_has_text and not right_has_text:
                # 如果三个地方都没有检查到文字，则是异常
                anomaly_files.append(filepath)

    # 输出封面和异常图片的路径和文件名
    with open('cover_results.txt', 'w') as f:
