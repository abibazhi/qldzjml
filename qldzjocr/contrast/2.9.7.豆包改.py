    ...
    for line in comparison_results:
        if "没有找到" in line:
            ...
            if match_info and img_path:
                # 处理多余的字符串
                img_path = img_path.replace("图片路径: ", "")
                refined_output += f"{sutra} - {match_info} - 图片路径: {img_path}\n"
                
                # 确认路径是相对于当前工作目录的相对路径
                abs_img_path = os.path.abspath(os.path.join(os.getcwd(), img_path))
                print(f"尝试复制图片: {abs_img_path}")
                
                if os.path.exists(abs_img_path):
                    shutil.copy(abs_img_path, temp_dir)
                else:
                    print(f"警告: 图片路径 {abs_img_path} 不存在.")
            else:
                refined_output += f"{sutra} - 没有找到\n"
        else:
            # 如果行中包含图片路径信息，则提取图片路径并复制图片
            ...
            if len(parts) == 2:
                ...
                if os.path.exists(abs_img_path):
                    ...
                    print(f"警告: 图片路径 {abs_img_path} 不存在.")
                refined_output += f"{sutra} - 图片路径: {img_path}\n"
            else:
