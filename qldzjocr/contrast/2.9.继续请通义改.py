...
def find_next_match(sutra_line, image_dict, start_from=0, match_threshold=5):
    """从指定位置开始查找下一个匹配项"""
    sutra_len = len(sutra_line)
    last_matched_index = 0
    
    for index in range(start_from, len(image_dict)):
        ...
        for j in range(max_length):
            if j < sutra_len and j < target_len and sutra_line[j] == target_text[j]:
                ...
                if count >= match_threshold:
                    return index, image_dict[index]['img_path'], j + 1  # 返回匹配索引、图片路径和结束位置
            else:
                count = 0
...
def refine_comparison(comparison_result_path, image_text_path, refined_output_path):
    ...
    image_dict, _ = build_image_dict(image_text_path)
    ...
    for line in comparison_results:
        if "没有找到" in line:
            ...
            match_index, img_path, next_search_start = find_next_match(sutra_line, image_dict, search_start)
            
            if match_index is not None:
                refined_output += f"{sutra_line} - 相似匹配 - {image_dict[match_index]['text']} - {img_path} (起始位置: {match_index})\n"
                search_start = match_index + next_search_start  # 更新搜索起点为最后一个匹配位置之后
            else:
