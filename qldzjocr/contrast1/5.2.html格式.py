def convert_result_to_html(result_file, html_output_file):
    ...
    # 准备HTML输出
    html_content = "<html><body><table border='1'><tr><th>文本</th><th>匹配到的标准文本</th><th>匹配到的行号</th></tr>"
    ...
    for line in lines:
        ...
        if "(匹配到的标准文本:" in line:
            ...
            html_content += f"<tr><td>{text_block}</td><td>{standard_text}</td><td>{line_number}</td></tr>"
        else:
            # 如果没有匹配信息，则只显示文本块
            text_block = line.split(", ", 1)[1] if ", " in line else line
            html_content += f"<tr><td>{text_block}</td><td>-</td><td>-</td></tr>"
