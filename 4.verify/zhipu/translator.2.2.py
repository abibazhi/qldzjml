        ...
        for line in f:
            ...
            try:
                ...
                if isinstance(translator, list):
                    # 如果是列表，尝试提取每个元素的字符串表示
                    translator = ', '.join(str(item) if isinstance(item, (str, int, float)) else json.dumps(item) for item in translator)
                elif isinstance(translator, dict):
                    # 如果是字典，尝试将其转换为字符串表示
...
if __name__ == "__main__":
    # 替换为你的实际文件路径
    excel_file_path = 'translator.0.*.xlsx'
