...
def check_translator_in_image(image_url, target_translator):
    prompt = (
        ...
        "2. 如果存在，请直接回答'是的'。\n"
        "3. 如果不存在，请尝试识别图片中的译者名称，并与目标译者进行逐字比较。\n"
        "4. 判断两个名字是否表示同一个译者（包括别名、异体字、拼写差异等情况）。\n"
        "5. 如果认为是同一个，请回复'是的'；否则回复'否'。\n"
        "6. 不管结果如何，请打印识别出的译者名称，并与目标译者逐字比较。\n"
        "7. 如果有多个译者，请逐一比较并说明哪个最接近。"
    )
...
def parse_ai_result(ai_response):
    if ai_response is None:
        return '调用失败'
    elif '是的' in ai_response or '一致' in ai_response:
        return '一致'
    elif '否' in ai_response or '不一致' in ai_response:
        return '不一致'
    else:
        return '未知'
...
if __name__ == "__main__":
    ...
    for idx, row in df.iterrows():
        ...
        judgment = parse_ai_result(ai_response)
